...
public:
    int numberOfStableArrays(int zero, int one, int limit) {
        ...
        // dp tables
        // dp_end_zero[i][j] -> arrays with i zeros, j ones, ending with 0
        // dp_end_one[i][j]  -> arrays with i zeros, j ones, ending with 1
        ...
        // initialize sequences containing only zeros
        for (int i = 1; i <= min(zero, limit); i++) {
            dp_end_zero[i][0] = 1;
        }

        // initialize sequences containing only ones
        ...
        // total arrays can end with either 0 or 1
